...
export default function LibraryTab({ books, session, genreMap, openAddModal, openEditModal, openRatingMode }) {
  const {
    filteredBooks,
    search, setSearch,
    ...
    <div>
      ...
      {subTab === "list" && (
        <div>
          ...
          {/* Rows — virtualized */}
          <div ref={scrollRef} className="lib-table-wrap" style={{ background: G.card, borderTop: "none", borderRadius: "0 0 8px 8px", maxHeight: 520, overflowY: "auto" }}>
            {filteredBooks.length === 0
              ? <div style={{ padding: 40, textAlign: "center", color: G.muted }}>No books match your filters.</div>
              : (
                <div style={{ height: virtualizer.getTotalSize(), position: "relative" }}>
                  {virtualizer.getVirtualItems().map(vRow => {
                    const b = filteredBooks[vRow.index];
                    const rm = b.rating ? RATING_META[b.rating] : null;
                    return (
                      <div key={b.id} data-index={vRow.index} ref={virtualizer.measureElement}
                        style={{ position: "absolute", top: 0, left: 0, width: "100%", transform: `translateY(${vRow.start}px)` }}>
                        <div className="lib-row" onMouseEnter={() => setHoveredBook(b)} onMouseLeave={() => setHoveredBook(null)}>
