...
import { AnalysisContext } from '../../src/contexts/AnalysisContext.jsx'
...
describe('AnalysisTab', () => {
  ...
  it('shows skeleton loader when booksLoading is true', () => {
    const { container } = setup({ booksLoading: true })
    expect(container.querySelectorAll('.pulse').length).toBeGreaterThan(0)
    expect(screen.queryByText('Volume & Pace')).toBeNull()
  })
  ...
  it('Save button in editing panel calls savePanelPromptsToSupabase and closes editor', async () => {
    const savePanelPromptsToSupabase = vi.fn().mockResolvedValue(true)
    const setEditingPanel = vi.fn()
