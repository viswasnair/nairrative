...
describe('db.saveAnalysisCache', () => {
  it('upserts with user_id, fingerprint, data and onConflict:user_id', () => {
    const chain = makeChain()
    supabase.from.mockReturnValue(chain)

    db.saveAnalysisCache('u-1', 'fp-abc', { temporal: { insight: 'x' } })

    expect(supabase.from).toHaveBeenCalledWith('analysis_cache')
    expect(chain.upsert).toHaveBeenCalledWith(
      { user_id: 'u-1', fingerprint: 'fp-abc', data: { temporal: { insight: 'x' } } },
      { onConflict: 'user_id' }
    )
  })

  it('payload does not contain a legacy id field', () => {
    const chain = makeChain()
    supabase.from.mockReturnValue(chain)

    db.saveAnalysisCache('u-1', 'fp', {})
...
describe('db.getAnalysisCache', () => {
  it('adds user_id eq filter when userId is provided', () => {
    const chain = makeChain()
    supabase.from.mockReturnValue(chain)

    db.getAnalysisCache('u-1')

    expect(chain.eq).toHaveBeenCalledWith('user_id', 'u-1')
    expect(chain.maybeSingle).toHaveBeenCalled()
  })

  it('skips user_id filter when userId is falsy', () => {
    const chain = makeChain()
    supabase.from.mockReturnValue(chain)

    db.getAnalysisCache(null)
...
describe('db.saveRecsCache', () => {
  it('upserts with user_id, fingerprint, data and onConflict:user_id', () => {
    const chain = makeChain()
    supabase.from.mockReturnValue(chain)

    db.saveRecsCache('u-2', 'fp-recs', { loved: [] })

    expect(supabase.from).toHaveBeenCalledWith('recs_cache')
    expect(chain.upsert).toHaveBeenCalledWith(
      { user_id: 'u-2', fingerprint: 'fp-recs', data: { loved: [] } },
      { onConflict: 'user_id' }
    )
  })
